...
`.claude/`에 정의된 멀티 에이전트 파이프라인 그 자체다. 여기서 "작업"이란 이 파이프라인을
따라 문서를 생성·검증·개정하고, 그 설계를 코드로 구현·검증하는 것을 뜻한다.

## 규칙

- 산출물 문서는 document 디렉터리에 저장한다.
- 에이전트는 `.claude/agents/`에 정의한다.
- 스킬은 `.claude/skills`에 정의한다.
...
모든 작업은 `.claude/agents/Orchestrator.md`에 정의된 상태 기계를 따른다.
...
## Skill이 절차를 정의한다 (`.claude/skills/<Role>/SKILL.md`)
