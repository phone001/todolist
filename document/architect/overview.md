...
| 근거 기획서 | `document/planner/plan.md` v1.15 |
| 작성 주체 | Architect |
...
| 버전 | 일자 | 변경 내용 |
| --- | --- | --- |
| v1.22 | 2026-09-17 | **설계 델타 — plan.md v1.15: F-19 재개정 — 워치 TodayView 상단 네비게이션 타이틀 문구 재변경 "오늘일정"→"오늘뭐해".** 사용자 신규 요청 원문: "`ios/TodayWhatWatch/TodayView.swift` 에서 네비게이션 타이틀을 '오늘일정'에서 '오늘뭐해'로 변경해줘" — v1.11 확정 후 v1.12·v1.13·v1.14/v1.20/v1.21(타이틀 유지)를 거치며 재확인된 값을 사용자가 명시적으로 재지시해 뒤집는 변경(되돌리기/regression 아님). 기획 재검증 **PASS** — 상세는 아래 "v1.22 재검증" 표. **범위 판단**: 상단 `NavigationBar` 타이틀 리터럴 1곳(`ios/TodayWhatWatch/TodayView.swift` 36행 `.navigationTitle("오늘일정")`)만 `.navigationTitle("오늘뭐해")`로 교체한다. 리스트 Section 헤더(69행 `Section(headerSummaryText)` — "오늘일정 - {완료}/{전체}", P-73/AC-96)는 상단 타이틀과 코드상 별개 UI 요소이므로 **변경하지 않는다** — 이번 변경으로 상단 타이틀="오늘뭐해" / 리스트 Section 헤더="오늘일정"으로 문구가 갈라지는 것은 의도된 결과다(plan.md v1.15 판단과 동일). `headerSummaryText` 계산 프로퍼티(산식 무변경)·다음 예정 미표시(P-68, v1.17)·체크박스 좌측 배치(P-69)·상단 헤더 요약 삭제(v1.20/v1.21, 이미 구현됨)는 이번 개정과 무관하며 전부 그대로 유지. **공유 페이로드 계약·전송 메커니즘·LWW·보류 큐·`ScheduleRow.swift`·DB 스키마(`database.md` v1.8)·보안 위협 모델은 전부 무변경** — 순수 하드코딩 문자열 리터럴 1곳 교체다. `logic.md` v1.22(§17.1/§17.9/§17.10/§17.11.3/§17.12/§13.8/§14/§15 갱신) / `nfr.md` v1.19(§14.5 검증 문구 중 상단 타이틀 확인 리터럴만 "오늘일정"→"오늘뭐해" 정정) 동반 개정. 「API 설계」 섹션 미신설(신규/외부 API 없음) |
| v1.21 | 2026-09-17 | **설계 전제 오류 정정 — Developer `BLOCKED(DESIGN_CONFLICT)` 대응(plan.md 델타 없음, 여전히 v1.14 기준).** Developer 가 `ios/TodayWhatWatch/TodayView.swift` 구현 착수 중, v1.20 설계(그 전제였던 v1.19 설계 포함)가 "P-73/AC-96(리스트 `Section` 헤더 `headerSummaryText` 요약 접미사)이 v1.19에서 이미 구현되어 있다"고 서술한 부분이 **사실과 다름**을 코드·git 워킹트리·`log/agent-execute.log` 대조로 확인해 보고했다. 실제로는 (1) `TodayView.swift` 70행이 지금도 `Section("오늘일정")`(접미사 없음)이고 `headerSummaryText` 는 34~38행 `.toolbar` 블록에서만 소비되며, (2) v1.13(P-73/AC-96) 라운드는 2026-09-16 Architect 디스패치 이후 Developer/Tester RESULT 로그가 없어 실제 구현·검증 사이클이 한 번도 완결되지 않았다 — v1.19 logic.md 설계가 다음 v1.20 설계의 전제로 이어지는 과정에서, 실제 코드 반영 여부를 재확인하지 않고 "이미 구현됨"으로 서술한 **Architect 문서화 공정 오류**였다(plan.md 의 요구사항 내용 자체는 문제 없음). **결정(Option B)**: P-73/AC-96 은 plan.md v1.13 에서 이미 "정식 반영"·"설계 착수 가능" 판정을 받은 확정·미변경 요구사항이고 설계(§17.12 `headerSummaryText` 재사용)도 이미 완결돼 있으므로, plan.md v1.14 의 "이번 개정 범위 밖으로 변경하지 않는다"(= 요구사항 **내용**을 바꾸지 않는다는 뜻이며 구현을 보류한다는 뜻이 아님)를 근거로 사용자가 명시적으로 요청한 v1.14(toolbar 삭제)와 **동일 Developer 라운드에서 함께 구현**하도록 지시를 명확화했다(범위 확장이 아니라 확정 요구사항의 뒤늦은 이행 — 미루면 (a) `headerSummaryText` 가 dead code 화, (b) AC-96 미충족 gap 이 새로 남는 두 문제가 생김). `logic.md` v1.21 — §17.1/§17.9/§17.10/§17.11.3 서술을 "v1.19/v1.20 설계, v1.21 구현"으로 정정하고, §17.12 를 "구현 순서(필수)" 절로 재작성: **1단계** `TodayView.swift` 70행 `Section("오늘일정")` → `Section(headerSummaryText)`(P-73/AC-96 최초 구현) → **2단계** 34~38행 `.toolbar { ToolbarItem(placement: .topBarTrailing) { Text(headerSummaryText) } }` 블록 완전 삭제(P-68, `.navigationTitle("오늘일정")` 만 유지). 순서 준수 이유(2단계 단독/선행 적용 시 dead code·AC-96 미충족 잔존)를 명시하고 현재 코드 기준선·최종 코드 상태를 코드 블록으로 첨부. **공유 페이로드 계약·전송 메커니즘·LWW·보류 큐·`ScheduleRow.swift`·DB 스키마(`database.md` v1.8)·보안 위협 모델은 전부 무변경** — 이번 정정은 설계 내용이 아니라 "그 설계가 실제 코드에 반영되었는가"에 대한 사실관계 정정과 Developer 재작업을 위한 순서 명확화다. `nfr.md` v1.18 — §14.5 검증 문구를 "무변경 회귀 확인"에서 "**최초 구현 확인**"(P-73/AC-96) + "toolbar 삭제 확인"(P-68) 동시 검증으로 정정. **신규 미결정 N-19** — 같은 세션 내에서 이전 설계 라운드의 실제 Developer 구현 완료 여부를 코드로 재확인하지 않고 다음 설계의 전제로 이어받는 공정 오류가 재발하지 않도록, 향후 Architect 라운드는 착수 전 대상 파일의 실제 코드 상태를 항상 우선 확인하도록 권장(비차단, Architect 공정 개선 사항). 아래 "v1.21 (설계 전제 오류 정정)" 영향 범위 절 참고. 「API 설계」 섹션 미신설(신규/외부 API 없음) |
| v1.20 | 2026-09-17 | **설계 델타 — plan.md v1.14: F-19 개정(축소) — 워치 TodayView 상단 헤더 요약(`.toolbar`) 삭제**(사용자 신규 요청: `ios/TodayWhatWatch/TodayView.swift` 34라인 근처 `.toolbar { ToolbarItem(placement: .topBarTrailing) { Text(headerSummaryText) } }` 제거, P-68 축소 개정 — "다음 예정 제거" 요건만 유지, 헤더 요약 요건 삭제. **AC-91 폐기** — 검증 대상 제외. AC-47/AC-54 재정정. E-19-5/E-19-8 문구 갱신). 기획 재검증 **PASS** — 상세는 아래 "v1.20 재검증" 표(plan.md AC-96 잔존 서술 1건 발견 — note, 비차단). 주요 결정: `ios/TodayWhatWatch/TodayView.swift` 루트 뷰의 `.navigationTitle("오늘일정")`(v1.11 확정 문구, 유지)에 붙어 있던 `.toolbar { ToolbarItem(placement: .topBarTrailing) { Text(headerSummaryText) } }` 블록을 **완전히 제거**한다 — 화면 상단에는 "오늘일정" 타이틀만 남고 우측 요약은 표시되지 않는다. 계산 프로퍼티 `headerSummaryText`(산식 무변경)는 삭제하지 않고 존치 — v1.19에서 도입된 리스트 Section 헤더(`Section(headerSummaryText)`, P-73)가 유일한 소비처가 된다(상단 헤더 소비처만 제거). 다음일정 제거(P-68, v1.17)·오늘 목록 Section 라벨(v1.18)·체크박스 좌측 배치(P-69, v1.17)·리스트 Section 헤더 요약(P-73, v1.19)은 이번 개정과 무관하며 그대로 유지 — plan.md v1.14 가 "리스트 Section 헤더는 코드상 별개 컴포넌트이므로 범위 밖"이라 명시한 판단과 동일하다. E-19-5(0건 케이스)는 "상단 헤더 요약 자체가 없어져 0건 케이스에는 상단·리스트 Section 헤더 어디에도 요약이 표시되지 않는다"로 정정, E-19-8 의 적용 대상을 "상단 헤더 요약"에서 "리스트 Section 헤더 요약(P-73)"으로 변경. AC-47/AC-54 는 상단 헤더 관련 Then 절을 "타이틀만 표시"로 정정, **AC-91 은 폐기(검증 대상 제외)**. **공유 페이로드 계약·`WatchSnapshot`/`WatchScheduleItem`·전송 메커니즘·LWW·보류 큐·`ScheduleRow.swift`는 전부 무변경.** **DB 스키마 변경 없음**(`database.md` v1.8 유지). **정량 NFR 목표 영향 없음**(계산·조회·전송 경로 무변경, 오히려 렌더 대상 1개 감소) — 다만 `nfr.md` v1.17 로 갱신하여 §9 V-58 검증 문구(상단 헤더 요약 스크린샷 확인 → 삭제 확인 + 리스트 Section 헤더 요약 회귀 확인)와 §14.5 를 실제 설계와 일치시켰다. **보안 위협 모델 영향 없음**(노출 위치가 2곳→1곳으로 줄어 오히려 축소). `logic.md` v1.20(§17.12 재작성·§17.9/§17.10/§17.11.3 갱신·§13.8/§14/§15 갱신) / `nfr.md` v1.17(§9 V-58, §14.5 갱신) 동반 개정. 「API 설계」 섹션 미신설(신규/외부 API 없음). **발견 사항(비차단, Planner 후속 정리 권장)**: plan.md AC-96(1779행·1781행)의 Then/And 절 2곳이 v1.14에서 삭제된 상단 헤더 요약이 "계속 존재"하는 것처럼 서술하고 있어, 같은 문서 내 F-19/P-68/E-19-5/E-19-8/AC-47/AC-54/AC-91/§7.10(모두 v1.14 갱신 지점, 8곳에서 일관되게 "삭제"로 서술)과 상충한다. 본 설계는 다수 지점의 v1.14 확정 서술(삭제 의도)을 따랐다 — plan.md AC-96 잔존 서술 정정은 Architect 권한 밖이므로 수정하지 않고 아래 「미결정 사항」 N-18 에 기록한다 |
| v1.19 | 2026-09-16 | **설계 델타 — plan.md v1.13: F-19 개정 — 워치 TodayView 리스트 Section 헤더에 완료/전체 요약 접미사 추가**(사용자 신규 요청, P-73/AC-96 신규). 기획 재검증 **PASS** — 상세는 아래 "v1.19 재검증" 표. 주요 결정: `ios/TodayWhatWatch/TodayView.swift`의 `Section("오늘일정")`(오늘 일정 리스트, 할일 카드들 바로 위 헤더 — 상단 `.navigationTitle`/`.toolbar` 헤더 요약(P-68)과는 별개 뷰 요소) 타이틀을 `Section(headerSummaryText)`로 교체해 "오늘일정 - {완료}/{전체}"를 표시한다. **신규 계산 없음** — 상단 헤더가 이미 산출해 둔 계산 프로퍼티 `headerSummaryText`(`snapshot?.summary.done`/`notDone` 기반)를 그대로 재사용해, 상단 헤더와 리스트 Section 헤더가 항상 동일한 문자열을 표시하도록 설계했다(DRY, 두 위치 값 불일치 가능성을 원천 차단). 오늘 일정이 0건이면(`today.isEmpty`) 이 `Section` 자체가 렌더되지 않고 빈 상태 Section으로 대체되는 기존 분기가 그대로 적용되어, "0건일 때 리스트 Section 헤더 접미사 미표시"(plan.md E-19-5) 요건이 별도 코드 없이 자동 충족된다. **공유 페이로드 계약(`WatchSnapshot`)·전송 메커니즘·`ScheduleRow.swift`·상단 헤더·RN 앱은 전부 무변경.** **DB 스키마 변경 없음**(`database.md` v1.8 유지). **NFR 영향 없음**(`nfr.md` v1.16 유지 — 신규 계산·조회 없음). **보안 위협 모델 영향 없음**(이미 노출 중인 `summary` 값을 한 곳 더 표시할 뿐, 노출 정보량 불변). `logic.md` v1.19(§17.12 갱신·§17.9/§17.10/§13.8/§14 갱신) 동반 개정. 「API 설계」 섹션 미신설(신규/외부 API 없음) |
| v1.18 | 2026-09-15 | **설계 델타 — plan.md v1.12: (1) v1.11 확정 워치 TodayView 타이틀·헤더 요약 문구 "오늘"→"오늘일정" 반영(직전 라운드 사용자 작업취소로 RESULT 없이 중단되어 logic.md v1.17 에 미반영 상태였던 것을 발견·수정) / (2) v1.12(A) RN 대시보드 탭 라벨·네비게이션 타이틀 "오늘"→"오늘일정" 신규 반영(OI-32 CLOSED, P-72/AC-95) / (3) v1.12(B) 워치 레이아웃 검증 결과(변경 불요) 확인.** 기획 재검증 **PASS** — 상세는 아래 "v1.18 재검증" 표. plan §11 OI 레지스트리의 OI-32 문구 미동기화(v1.11 시점 "사용자 확인 대기" 문구 잔존, CLOSED 미표기)·OI-33 목록 미등재는 **note 로 기록하고 설계를 차단하지 않음** — 변경이력 표(v1.12 행)·F-10 서술부·§5.20(P-72) 서술에 "OI-32 CLOSED"·"OI-33 신규"가 명확히 기술되어 실질 정책 내용은 모호하지 않기 때문(§11 자체는 Architect 가 수정할 문서가 아니므로 Planner 후속 정리 권장만 남김). 주요 결정: 전부 **순수 리터럴 문자열 교체** — (1) `ios/TodayWhatWatch/TodayView.swift` 의 `.navigationTitle("오늘")`→`.navigationTitle("오늘일정")`, `headerSummaryText` 접두어 "오늘 - "→"오늘일정 - "(P-68, AC-91, E-19-8), `Section("오늘")`→`Section("오늘일정")`(설계 판단, 비필수 확장 — AC 미고정 순수 레이아웃 라벨). 빈 상태 문구 "오늘 일정이 없습니다"(E-19-5)·`ScheduleRow.swift` 는 **무변경**(plan.md AC 문구 불변, "오늘"이 날짜 지시어). (2) `src/app/navigation/RootNavigator.tsx` 의 `title: '오늘'`→`title: '오늘일정'`, `TAB_FALLBACK_LABELS[TAB_ROUTES.Dashboard] = '오늘'`→`'오늘일정'`(P-72, AC-95, OI-32 CLOSED). `DashboardScreen.tsx` 내부의 "오늘로" 복귀 버튼·`오늘 · {date}` 날짜 배지(F-20)·빈 상태 문구(`dashboardViewModel.ts`)는 **이번 개정에서 제외(OI-33, 설계 결정)** — 날짜 지시어(당일)로 쓰이는 "오늘"이라 탭 명칭과 무관하고 plan.md AC 문구도 불변이며, 바꾸면 "오늘일정로"처럼 문법이 부자연스러워짐. (3) plan.md F-19 「레이아웃 검증 근거 (v1.12)」의 "완전히 일치, 변경 불요" 결론에 설계 관점 이견 없음 — `nfr.md` §14.5 에 회귀 참고자료 1줄만 추가, 별도 설계 변경 없음. **DB 스키마 변경 없음**(`database.md` v1.8 유지 — 순수 문자열 리터럴이라 값 도메인·구조 무관). **보안 위협 모델 영향 없음**(하드코딩 정적 문자열 교체, 사용자 입력·신뢰 경계 무관). `logic.md` v1.18(§16.2 신설 소단락·§17.3/§17.9/§17.10/§17.11.3/§17.12 문구 정정·§13.3/§13.8/§14/§15 갱신) / `nfr.md` v1.16(§9 V-58·§14.5 문구 정정 + 회귀 참고자료 1줄) 동반 개정. 「API 설계」 섹션 미신설(신규/외부 API 없음) |
...
### v1.22 재검증 (plan.md v1.15 — F-19 재개정: 워치 TodayView 상단 네비게이션 타이틀 문구 재변경 "오늘일정"→"오늘뭐해")

**결과: PASS (설계 가능, Planner 재작업 불필요)**

| 검증 항목 | 판정 | 비고 |
| --- | --- | --- |
| 원본 요청/상위 요구사항 반영 | OK | 사용자 신규 요청 원문 "`ios/TodayWhatWatch/TodayView.swift` 에서 네비게이션 타이틀을 '오늘일정'에서 '오늘뭐해'로 변경해줘"가 F-19 「표시 요소」 절 재개정·P-68 재개정(§5.19)·AC-47/AC-54 재정정·§7.10(2·3단계)·plan §10 매핑 신규 행에 등재되어 있다 |
| 기능 목적/범위/시나리오 | OK | 변경 대상이 상단 `NavigationBar` 타이틀 리터럴(`ios/TodayWhatWatch/TodayView.swift` 36행 `.navigationTitle("오늘일정")`) **1곳**으로 코드 라인까지 특정되어 있고, 리스트 Section 헤더(P-73/AC-96, v1.13)의 리터럴 "오늘일정"은 "상단 타이틀과 코드상 별개 UI 요소이므로 변경 범위 밖"이라고 plan.md가 명시적으로 판단 근거를 남겨 모호함이 없다. 두 요소의 문구가 이번 변경으로 갈라지는 것(상단="오늘뭐해"/리스트 Section="오늘일정")이 의도된 결과임도 plan.md가 명시 |
| 입력·출력·상태변화·예외 | OK | AC-47/AC-54 가 Then 절에서 상단 타이틀 리터럴을 "오늘뭐해"로 갱신하고, 리스트 Section 헤더 요약(AC-96)의 "오늘일정" 리터럴은 그대로 유지됨을 명시 — 검증 가능한 형태. E-19-5 의 0건 케이스 결론(상단·리스트 어디에도 완료/전체 요약 미표시)은 이번 개정과 무관하게 무변경 |
| 비기능 요구사항 | OK | 신규 정량 SLO 없음. 순수 문자열 리터럴 1곳 교체로 성능·용량·가용성 영향 없음 |
| 요구사항 충돌 | 없음 | plan.md가 F-19 서술부·P-68·AC-47·AC-54·§7.10·§10 매핑에서 일관되게 "상단 타이틀만 재변경, 리스트 Section 헤더는 유지"를 서술 — 상호 모순 없음. plan.md 는 같은 라운드에서 AC-96의 v1.14 당시 잔존 서술(상단 헤더 요약이 여전히 존재하는 것처럼 보이던 문장, v1.20 재검증 note 항목)도 함께 정정해 이전 발견 사항(N-18)도 해소되었다 |
| 가정/확정 구분 | OK | "확정"(원문 반영). 신규 미결정 게이트 없음 |
| Acceptance Criteria 검증 가능성 | OK | AC-47/AC-54(재정정)·AC-96(무변경, 계속 검증) 전부 Given/When/Then 이 구체적이라 정적 리뷰·회귀 테스트로 검증 가능 |
| 미결정 게이트 영향 | **비차단** | 신규 게이트 없음 |

**설계 반영 요약**: `logic.md` §17.1(장 제목)·§17.9(E-19-5)·§17.10(AC-47/AC-54 재정정)·§17.11.3(`TodayView.swift` 행 정정)·§17.12(신규 "v1.22 개정" 소단락 — 리터럴 1곳 교체 지시, 현재/최종 코드 상태 코드 블록)·§13.8(위협표 1행 추가)·§14/§15(일관성 갱신)에 반영했다. `database.md`는 순수 문자열 리터럴 변경이라 **변경 불필요**(값 도메인·데이터 구조 전부 무관). `nfr.md` 는 **v1.19로 갱신** — §14.5 검증 문구 중 상단 타이틀 확인 리터럴만 "오늘일정"→"오늘뭐해"로 정정(리스트 Section 헤더 확인 리터럴 "오늘일정 - M/N"은 무변경, 정량 목표·인덱스·의존성 변경 없음).

---

### v1.20 재검증 (plan.md v1.14 — F-19 개정(축소): 워치 TodayView 상단 헤더 요약(`.toolbar`) 삭제)

**결과: PASS (설계 가능, Planner 재작업 불필요) — 단, plan.md 자체 내부에 잔존 서술 1건 발견(note, 비차단)**

| 검증 항목 | 판정 | 비고 |
| --- | --- | --- |
| 원본 요청/상위 요구사항 반영 | OK | 사용자 신규 요청 원문 "`ios/TodayWhatWatch/TodayView.swift` 34라인 근처의 `.toolbar { ToolbarItem(placement: .topBarTrailing) { Text(headerSummaryText) } }`(상단 타이틀 우측 헤더 요약 "오늘일정 - {완료}/{전체}")를 제거해 달라"가 F-19 「표시 요소」 절 개정·P-68 축소 개정(§5.19)·AC-91 폐기·AC-47/AC-54 재정정·E-19-5/E-19-8 문구 갱신·§7.10(3·5단계)·plan §10 매핑 신규 행에 등재되어 있다 |
| 기능 목적/범위/시나리오 | OK | 대상 UI 요소가 코드 라인·SwiftUI 조각(`ToolbarItem(placement: .topBarTrailing)`)까지 지칭되어 **상단 NavigationBar 툴바**로 정확히 특정되어 있고, 리스트 Section 헤더 요약(P-73, v1.13)은 "코드상 별개 컴포넌트이므로 범위 밖"이라고 plan.md가 명시적으로 판단 근거를 남겨 모호함이 없다 |
| 입력·출력·상태변화·예외 | OK | AC-47/AC-54 가 "타이틀만 표시, 우측 요약 없음" + "리스트 Section 헤더 요약은 별도로 계속 표시" Then 절로 구체적. E-19-5 는 0건 케이스에 "상단·리스트 Section 헤더 어디에도 요약 미표시"로 명확. E-19-8 은 적용 대상을 리스트 Section 헤더로 정확히 재지정 |
| 비기능 요구사항 | OK | 신규 정량 SLO 없음. UI 요소 1개를 제거하는 순수 축소 변경으로 성능·용량·가용성 영향 없음(오히려 렌더 대상 감소) |
| 요구사항 충돌 | 없음 | plan.md가 F-19/P-68/E-19-5/E-19-8/AC-47/AC-54/AC-91/§7.10 8곳에서 일관되게 "상단 헤더 요약 삭제"를 서술 — 상호 모순 없음(단, AC-96 잔존 서술은 아래 note 참고) |
| 가정/확정 구분 | OK | "확정"(원문 반영). 신규 미결정 게이트 없음 |
| Acceptance Criteria 검증 가능성 | OK | AC-47/AC-54(재정정)·AC-91(폐기 — 검증 대상 제외, 사유 명시)·AC-96(무변경, 계속 검증) 전부 Given/When/Then 또는 폐기 사유가 명확해 정적 리뷰·회귀 테스트로 검증 가능 |
| 미결정 게이트 영향 | **비차단** | 신규 게이트 없음 |

**note — plan.md 자체 내부 잔존 서술 발견 (Architect 수정 범위 밖, 설계를 차단하지 않음)**: plan.md AC-96(v1.13 신규 항목, 1779행·1781행)의 Then/And 절 2곳이 — "상단 네비게이션 타이틀 옆 헤더 요약("오늘일정 - 2/3", P-68)도 동일한 수치로 함께 표시된다"(1779행), "이 경우 요약은 상단 헤더의 '오늘일정 - 0/0' 하나뿐이다"(1781행) — 이번 v1.14 개정으로 **삭제된 상단 헤더 요약이 여전히 존재하는 것처럼** 서술하고 있어, 같은 문서 내 F-19 서술부·P-68·E-19-5·E-19-8·AC-47·AC-54·AC-91·§7.10(모두 v1.14 갱신 지점, 8곳에서 일관되게 "상단 헤더 요약 삭제"로 서술)과 상충한다. Planner의 v1.14 변경 이력에는 "P-73/AC-96(v1.13)은 이번 개정과 무관하며 그대로 유지"라고만 적혀 있어 AC-96 본문 자체의 교차 참조 문장이 갱신되지 않은 것으로 판단된다. 본 설계는 8곳에서 반복 확인되는 v1.14 확정 서술(삭제 의도, 사용자 원문 요청과도 일치)을 따라 상단 헤더 요약을 제거하는 방향으로 설계했다 — **AC-96 잔존 서술 자체의 정정은 Architect 권한 밖(Planner 소관)이므로 plan.md 를 수정하지 않았다.** 아래 「미결정 사항」 N-18 에 기록해 Planner 후속 정리를 권장한다.

**설계 반영 요약**: `logic.md` §17.12(재작성 — 상단 헤더 요약 삭제, `headerSummaryText` 존치·소비처 1곳으로 축소)·§17.9(E-19-5/E-19-8 갱신)·§17.10(AC-47/AC-54 재정정, AC-91 폐기, AC-96 갱신)·§17.11.3(`TodayView.swift` 행 정정)·§13.8(위협표 1행 추가)·§14/§15(일관성·미결정 갱신)에 반영했다. `database.md`는 순수 UI 요소 제거라 **변경 불필요**(값 도메인·데이터 구조 전부 무관). `nfr.md` 는 **v1.17로 갱신** — §9 V-58 이 이제는 존재하지 않는 상단 헤더 요약 스크린샷 확인을 지시하고 있어 검증 문구가 실제와 불일치하게 되므로, 검증 대상을 "상단 요약 부재 확인 + 리스트 Section 헤더 요약(P-73/AC-96) 존재 확인"으로 정정하고 §14.5 에 v1.17 문단을 추가했다(정량 목표·인덱스·의존성 변경은 없음, 순수 검증 체크리스트 정정).

---

### v1.19 재검증 (plan.md v1.13 — F-19 개정: 워치 TodayView 리스트 Section 헤더에 완료/전체 요약 접미사 추가)

**결과: PASS (설계 가능, Planner 재작업 불필요)**

| 검증 항목 | 판정 | 비고 |
| --- | --- | --- |
| 원본 요청/상위 요구사항 반영 | OK | 사용자 신규 요청 원문 "워치화면에서 할일카드 바로위의 '오늘할일'이라는 타이틀 오른쪽에 '- 완료건수/총할일건수'를 추가해줘"가 F-19 「표시 요소」 절 신규 불릿·신규 P-73(§5.19)·신규 AC-96·plan §10 매핑에 등재되어 있다 |
| 기능 목적/범위/시나리오 | OK | 대상 UI 요소가 실제 코드(`ios/TodayWhatWatch/TodayView.swift`)까지 대조되어 **리스트 Section 헤더**(`Section("오늘일정")`, 70행 부근)로 정확히 특정되어 있고, 상단 `.navigationTitle`/`.toolbar` 헤더 요약(P-68)과 별개 요소임이 plan.md에 명시되어 모호함이 없다 |
| 입력·출력·상태변화·예외 | OK | AC-96 이 Given(3건 중 2건 완료)/When(워치 앱 열기)/Then("오늘일정 - 2/3" 표시 + 토글 시 갱신 + 0건이면 미표시) 형태로 구체적. E-19-5 도 "리스트 Section 자체가 렌더되지 않아 접미사도 표시되지 않는다"로 0건 케이스를 명확히 보강 |
| 비기능 요구사항 | OK | 신규 정량 SLO 없음. 상단 헤더가 이미 계산한 값을 한 곳 더 표시하는 순수 뷰 변경이라 성능·용량·가용성 영향 없음 |
| 요구사항 충돌 | 없음 | 상단 헤더(P-68)와 리스트 Section 헤더(P-73)에 동일 형식 요약이 동시에 표시되는 것을 plan.md가 "사용자가 리스트 Section 헤더를 명시적으로 지칭했으므로 의도된 결과"로 명시해 중복처럼 보일 수 있는 지점에 대한 판단 근거를 남겼다 — 모순 아님 |
| 가정/확정 구분 | OK | "확정"(원문 반영). 신규 미결정 게이트 없음 |
| Acceptance Criteria 검증 가능성 | OK | AC-96 전부 Given/When/Then 형태로 정적 리뷰·(가능하면) 스냅샷 테스트로 검증 가능 |
| 미결정 게이트 영향 | **비차단** | 신규 게이트 없음 |

**설계 반영 요약**: `logic.md` §17.12(리스트 Section 헤더 요약 접미사, `headerSummaryText` 재사용)·§17.9(E-19-5 보강)·§17.10(AC-96 추가)·§13.8(위협 요약 1행 추가)·§14(일관성 검증 행 추가)에 반영했다. `database.md`/`nfr.md`는 순수 UI 표시 문구 변경이라 **변경 불필요**로 판단했다(값 도메인·데이터 구조·성능·용량 전부 무관).

---

### v1.18 재검증 (plan.md v1.12 — (1) v1.11 확정 워치 TodayView 타이틀·헤더 요약 문구 반영 / (2) v1.12(A) RN 대시보드 탭 라벨 확정 / (3) v1.12(B) 워치 레이아웃 검증 결과)

**결과: PASS (설계 가능, Planner 재작업 불필요)**

| 검증 항목 | 판정 | 비고 |
| --- | --- | --- |
| 원본 요청/상위 요구사항 반영 | OK | (1) v1.11 이 이미 워치 타이틀·헤더 요약 문구 변경을 확정(F-19/P-68/E-19-5/E-19-8/AC-47/AC-54/AC-91/§7.10). (2) v1.12(A) 사용자 신규 요청 원문 "앱에서 메뉴 중 오늘을 오늘일정으로 변경해줘"가 F-10/F-16 서술부·신규 §5.20(P-72)·신규 AC-95·AC-26 정정으로 plan §10 매핑에 등재. (3) v1.12(B) 스크린샷 검증 결과가 F-19 절에 「레이아웃 검증 근거 (v1.12)」 문단으로 명시 |
| 기능 목적/범위/시나리오 | OK | (1)(2) 모두 변경 범위가 **정확히 2개 리터럴씩**(워치: `.navigationTitle`/`headerSummaryText` 접두어, RN: `title`/`TAB_FALLBACK_LABELS`)으로 코드 파일·라인까지 특정되어 모호함이 없다. (3) 레이아웃 항목 8개가 나열되어 있고 "변경 불요"라는 결론과 "코드 변경 지시 없음"이 명시적으로 일치 |
| 입력·출력·상태변화·예외 | OK | E-19-5/E-19-8(문구 갱신, 0건 케이스 "오늘일정 - 0/0")·AC-47/AC-54/AC-91(정정) 모두 Given/When/Then 이 구체적. RN 쪽 AC-95 도 "탭 라벨/네비게이션 타이틀 둘 다 '오늘일정'" + "폴백 텍스트도 '오늘일정'"로 검증 가능 |
| 비기능 요구사항 | OK | 신규 정량 SLO 없음. 순수 문자열 리터럴 교체라 성능·용량·가용성 영향 없음 |
| 요구사항 충돌 | 없음 | v1.12 변경 이력 표·§5.20(P-72) 는 "OI-32 CLOSED"·"OI-33 신규"를 명확히 서술 — F-10/F-16/§7.2/AC-95/AC-26 정정과 모순 없이 일관됨. (1) 워치 타이틀 변경은 v1.11 에서 이미 확정되어 있었고 이번 라운드가 그 반영 누락을 뒤늦게 해소하는 것일 뿐 신규 정책 결정이 아니다 |
| 가정/확정 구분 | OK | (1)(2)(3) 전부 "확정"(원문 반영, 신규 게이트 없음). 신규 미결정 게이트는 생성되지 않았다 |
| Acceptance Criteria 검증 가능성 | OK | AC-47/AC-54/AC-91(워치 타이틀·헤더 요약 리터럴)·AC-95(RN 탭 라벨·폴백 텍스트)·AC-26(탭 아이콘 표 정정) 전부 Given/When/Then 형태로 정적 리뷰·단위 테스트 검증 가능 |
| 미결정 게이트 영향 | **비차단** | 신규 게이트 없음. OI-33(DashboardScreen 내부 다른 "오늘" 표현 확장 여부)은 plan 이 처음부터 Architect/Developer 판단 위임으로 명시한 비차단 사항이며, 이번 라운드에서 "제외" 로 판단하고 근거를 남겼다(§16.2 신설 소단락) |

**§11 OI 레지스트리 동기화 이슈 — note (설계 차단 아님)**: plan.md §11(열린 이슈 목록, OI-32 항목 부근)이 v1.12 갱신에서 동기화되지 않았다. 구체적으로 (a) OI-32 항목이 v1.11 시점 문구("이번 v1.11 개정은 워치 한정… RN 앱 쪽은 이번 개정에서는 변경하지 않는다… 사용자 확인 대기")로 그대로 남아 있고 **CLOSED 로 갱신되지 않았다**, (b) 신규 **OI-33** 이 §11 목록 자체에 항목으로 추가되지 않았다(변경이력 표·F-10 서술부·§5.20 서술에는 등장하나 §11 리스트에는 없음). 그러나 이 미동기화는 **§11이라는 "열린 이슈 추적 목록"의 갱신 누락**일 뿐이며, 정책 내용 자체는 변경이력 표(v1.12 행)·F-10 서술부·§5.20(P-72) 서술 3곳에서 "OI-32 CLOSED(본 정책으로 해소)"·"OI-33 신규(비차단, Architect/Developer 위임)"로 **일관되고 명확하게** 기술되어 있어 설계 착수에 필요한 정책 판단이 모호하지 않다. 따라서 이번 검증은 이를 **FAIL 사유로 취급하지 않고 PASS 판정 + note** 로 남긴다 — Planner 에게 다음 라운드에서 §11 목록 자체를 동기화(OI-32 CLOSED 표기, OI-33 항목 추가)하도록 권장하되, 이번 Architect/Developer 작업을 되돌리지는 않는다.
...
| F-19 개정 — 워치 다음일정 제거 · 체크박스 좌측 배치 (v1.10/v1.11), P-68/P-69, D-30 | `TodayView.swift` — "완료/미완료" `Section` 헤더 + "다음 예정" `Section` 을 제거하고 `NavigationStack`/`.navigationTitle("오늘일정")` 적용. `Section("오늘")`→`Section("오늘일정")`(v1.18, 설계 판단). 빈 상태 문구·`ScheduleRow.swift` 는 무변경(OI-33 원칙 준용). `ScheduleRow.swift` — 완료 토글 `Button` 을 행 좌측으로 이동. `WatchSyncService.pushSnapshot()` 이 다음 예정 조회를 중단하고 `nextUpcoming=null` 고정 전달(D-30(a)). **공유 타입(`WatchSnapshot`) 무변경** — Swift `Models.swift` 변경 없음. **(상단 헤더 요약 관련 서술은 v1.20에서 삭제되어 아래 별도 행으로 이관됨)**. `logic.md` §17.3·§17.12 |
| F-19 개정 — 워치 상단 헤더 요약(`.toolbar`) 삭제 (v1.14, plan.md), P-68(축소 개정), ~~AC-91~~(폐기) | 사용자 신규 요청(`TodayView.swift` 34라인 근처 `.toolbar { ToolbarItem(placement: .topBarTrailing) { Text(headerSummaryText) } }` 제거)에 따라 이 블록을 완전히 삭제 — `.navigationTitle("오늘일정")`만 남긴다(**v1.22 부터 "오늘뭐해"**, 아래 행 참고). 계산 프로퍼티 `headerSummaryText`(산식 무변경)는 존치되어 리스트 Section 헤더 요약(P-73)의 유일한 소비처가 된다. **v1.21 정정**: 리스트 Section 헤더 요약(P-73/AC-96)은 이번 개정과 무관하지만 v1.21 조사 결과 **아직 구현된 적이 없음이 확인**되어, 이 delete와 **함께 처음 구현**한다(순서: 1단계 `Section(headerSummaryText)` 전환 → 2단계 `.toolbar` 삭제). 다음일정 제거(P-68)·체크박스 좌측 배치(P-69)는 이번 개정과 무관하게 그대로 유지. **공유 타입·전송 메커니즘 무변경**. `logic.md` §17.12 |
| F-19 재개정 — 워치 상단 네비게이션 타이틀 재변경 "오늘일정"→"오늘뭐해" (v1.15, plan.md, 사용자 명시적 재지시), P-68(재개정) | `TodayView.swift` **36행** `.navigationTitle("오늘일정")` → `.navigationTitle("오늘뭐해")` 리터럴 1곳만 교체. 리스트 Section 헤더(69행 `Section(headerSummaryText)`, "오늘일정 - {완료}/{전체}", P-73/AC-96)는 상단 타이틀과 코드상 별개 UI 요소이므로 **변경하지 않음** — 두 요소 문구가 갈라지는 것은 의도된 결과. `headerSummaryText` 산식·`.toolbar` 삭제 상태(v1.20/v1.21)·다음일정 제거(P-68)·체크박스 좌측 배치(P-69)는 전부 무변경. **공유 타입·전송 메커니즘·DB 스키마 무변경**. `logic.md` §17.12 |
...
| F-10/F-16 개정 — 대시보드 탭 라벨·네비게이션 타이틀 "오늘" → "오늘일정" (v1.12, OI-32 CLOSED), P-72, AC-95 | `RootNavigator.tsx` 의 `title: '오늘'` → `title: '오늘일정'`, `TAB_FALLBACK_LABELS[TAB_ROUTES.Dashboard] = '오늘'` → `'오늘일정'` **정확히 2개 리터럴**. 탭 아이콘(`todo.png`) 매핑 무변경. `DashboardScreen.tsx` 내부 다른 "오늘" 표현(날짜 네비게이션·빈 상태 문구)은 이번 라운드 제외(OI-33, 설계 결정 — 날짜 지시어이며 plan.md AC 문구 불변). `logic.md` §16.2 |
...
- `ios/TodayWhatWatch/` — **단일 타깃 watchOS 앱**(SwiftUI `App` 라이프사이클 + `WKApplicationDelegateAdaptor`, watchOS 10.0+). 레거시 "WatchKit App + WatchKit Extension" 2-타깃 구조는 채택하지 않는다(Xcode 14+ 표준). SwiftUI 뷰(오늘 목록·좌측 완료 토글(v1.17, P-69)·"오늘일정" 타이틀 + 우측 헤더 요약 "오늘일정 - M/N"(v1.17 신설/v1.18 문구 정정, P-68 — 기존 "완료/미완료" 카운트·"다음 예정" 행 대체)·빈 상태 E-19-5·"최신 아님" E-19-1·"동기화 대기" D-11 배지·pull-to-refresh), `WCSessionDelegate`, 로컬 스냅샷/보류 큐 JSON 영속화(앱 컨테이너, `FileProtectionType.complete`), LWW·상태 규칙(§7.6)을 Swift 로 재구현. **`src/core` 코드를 링크하지 않는다** — 공유 자산은 페이로드 계약(`src/core/watchSync/types.ts`·`src/app/adapters/watch/watchMessage.ts`)·시각 규약(P-39)뿐이다.
...
| 워치 상단 헤더 요약 삭제 — **해당 없음(노출면 축소)** (F-19, v1.20 설계·v1.21 구현) | 이미 두 위치(상단 헤더 + 리스트 Section 헤더)에 노출되던 `summary.done/notDone` 값이 이제 리스트 Section 헤더(P-73) 한 곳에서만 노출된다 — 신규 필드·신규 조회·신규 로그 표면 없음, 오히려 노출 위치가 줄어든다. 상세 `logic.md` §13.8 |
| v1.21 설계 전제 오류 정정(구현 순서 명확화) — **해당 없음** | 위 항목의 설계 내용 자체는 무변경, "언제 구현되는가"만 정정. 신규 필드·조회·신뢰 경계 없음 |
| 워치 상단 타이틀 재변경 "오늘일정"→"오늘뭐해" — **해당 없음** (F-19, v1.22, plan.md v1.15) | 하드코딩 정적 문자열 리터럴 1곳(`TodayView.swift` 36행 `.navigationTitle`) 교체뿐 — 산출 소스·전송 계약·신뢰 경계 무변경. 리스트 Section 헤더 요약(P-73)의 "오늘일정" 리터럴은 이번 개정과 무관하게 그대로 유지. 신규 필드·조회·저장·로그 표면 없음 |
...
| `nfr.md` | 성능/용량/가용성/관측성 목표의 기술적 구체화, Tester 검증 관점, **§11 셸 검증 관점(v1.1)**, **§13 로딩 애니메이션 성능(v1.5, NFR-11)**, **§14 워치 동기화(v1.7, F-19 / NFR-10·NFR-12) + §11.1 로 watchOS 앱 타깃 빌드·시뮬레이터 설치/실행/WCSession 라운드트립/스크린샷 이동(v1.12, N-11 축소)**, **§15 대시보드 날짜 네비게이션·진행률 한 줄·접이식 검색(v1.8 신설 / v1.9 재확정 방향, F-20~F-22)**, **§16 통계 화면(v1.10 신설 / v1.11 막대→선 갱신 — 차트 렌더(§16.2 `react-native-svg` `Polyline`)·대량 집계 비용·접근성/스크린리더 대체 텍스트·Reduce Motion·i18n, F-23 / NFR-04·NFR-08·NFR-09, V-46~V-49)**, **§7 접근성 대비표(v1.13, 우선순위/유형 색상 대비) + §9 V-50~V-52(색상 자동배정·우선순위 매핑·프리셋 UI)**, **§17 반복 회차 실체화·완료 정렬·숨기기·시딩·캘린더 프리필(v1.14 신설, F-24/F-10/F-25/F-06/F-26) + §9 V-53~V-57 + §1.2 반복 설계 갱신(가상 전개 → 경계 내 실체화)**, **§14.3 워치 페이로드 구성 문구 정정(v1.15, F-19 — "다음 예정 1건" 삭제, D-30)**, **§9 V-58/§14.5 갱신 — 상단 헤더 요약 삭제 반영, 리스트 Section 헤더 요약 검증 문구 정정(v1.17, F-19/P-68/P-73)**, **§14.5 재정정 — "무변경 회귀 확인"→"P-73/AC-96 최초 구현 확인"(v1.18, v1.21 정정 대응)**, **§14.5 상단 타이틀 확인 리터럴 정정 "오늘일정"→"오늘뭐해"(v1.19, F-19/P-68, plan.md v1.15)** |

---

## 영향 범위

### v1.22 (F-19 재개정 — 워치 TodayView 상단 네비게이션 타이틀 재변경 "오늘일정"→"오늘뭐해", plan.md v1.15)

- **변경(Developer) — 네이티브 워치 타깃(`ios/TodayWhatWatch/`), `TodayView.swift` 1개 파일, 리터럴 1곳**:
  1. 착수 전 실제 코드가 아래 기준선과 일치하는지 재확인할 것(v1.21 구현이 이미 완료되어 있어야 함): 18~22행 `headerSummaryText` 계산 프로퍼티, 27~38행 `.navigationTitle("오늘일정")`(`.toolbar` 블록 없음), 69행 `Section(headerSummaryText)`.
  2. **36행** `.navigationTitle("오늘일정")` 을 `.navigationTitle("오늘뭐해")` 로 교체한다. 그 외 아무것도 변경하지 않는다.
- **변경하지 않음**: 계산 프로퍼티 `headerSummaryText`(18~22행, `"오늘일정 - \(done)/\(total)"` 산식) — 리스트 `Section` 헤더(69행)의 유일한 소비처로 그대로 유지. `.toolbar` 블록(v1.21에서 이미 제거됨), 다음 예정 미표시(P-68, v1.17), 체크박스 좌측 배치(P-69, `ScheduleRow.swift`).
- **최종 상태**: 화면 상단에는 `.navigationTitle("오늘뭐해")` 만 남고 우측 요약 없음(v1.21과 동일 레이아웃, 타이틀 리터럴만 교체). 할일 카드 목록 바로 위 리스트 `Section` 헤더에는 여전히 "오늘일정 - {완료}/{전체}"가 표시된다(P-73/AC-96, **무변경, 회귀 아님**).
- **무변경**: 공유 페이로드 계약(`WatchSnapshot`/`WatchScheduleItem`), `WatchSyncService`/`WatchConnectivityGateway`/`resolveToggleLWW`, `ScheduleService`/`CategoryService` 등 모든 서비스·검증 로직, `bindings.ts`, DB 스키마(`database.md` v1.8 유지), RN 앱(`src/app/**`) 전체.
- **DB 스키마 변경**: 없음 — `database.md` v1.8 유지.
- **신규 의존성**: 없음.
- **NFR 문서**: `nfr.md` v1.19 — 정량 목표 변경 없음, §14.5 검증 문구 중 상단 타이틀 확인 리터럴만 "오늘일정"→"오늘뭐해"로 정정(리스트 Section 헤더 확인 리터럴 "오늘일정 - M/N"은 무변경).
- **재검증 필요 AC (Developer 구현 후 Tester)**: **AC-47**(재정정 — 상단 타이틀 리터럴 "오늘뭐해" 확인) · **AC-54**(재정정 — 빈 상태에 "오늘뭐해" 타이틀만 표시). 회귀 확인: **AC-96**(리스트 Section 헤더 요약 "오늘일정 - {완료}/{전체}" — 무변경) · **AC-92**(다음일정 부재+체크박스 좌측 — 무변경) · **AC-53**(워치 페이로드 범위 — 문구·필드 무관).
- **회귀 위험**: 낮음 — 순수 문자열 리터럴 1곳 교체에 한정되며 상태·서비스·데이터 흐름·다른 UI 요소를 건드리지 않는다. 유일한 주의점은 리스트 Section 헤더가 참조하는 `headerSummaryText` 를 실수로 함께 수정하지 않는 것(수정 시 P-73/AC-96 회귀) — `logic.md` §17.12 "v1.22 개정" 참고.

### v1.21 (설계 전제 오류 정정 — Developer `BLOCKED(DESIGN_CONFLICT)` 대응, plan.md 델타 없음)

> **이 절이 아래 "v1.20" 절의 Developer 지시를 대체한다.** v1.20 절은 "v1.19에서 `Section(headerSummaryText)` 전환이 이미 구현되어 있다"는 잘못된 전제로 "`.toolbar` 블록만 삭제하면 된다"고 지시했으나, 실제 코드(`ios/TodayWhatWatch/TodayView.swift` 70행)는 지금도 `Section("오늘일정")`(접미사 없음)이다. Developer 는 아래 지시를 따른다.

- **변경(Developer) — 네이티브 워치 타깃(`ios/TodayWhatWatch/`), `TodayView.swift` 1개 파일, 2단계 순서 필수**:
  1. **1단계(P-73/AC-96 최초 구현)**: 70행 `Section("오늘일정") { ForEach(today) { ... } }` 의 `"오늘일정"` 을 계산 프로퍼티 `headerSummaryText`(18~22행, 기존 그대로 — 신규 계산 불필요)로 교체해 `Section(headerSummaryText) { ForEach(today) { ... } }` 로 만든다.
  2. **2단계(P-68 축소 개정, 1단계 이후 적용)**: 34~38행의 `.toolbar { ToolbarItem(placement: .topBarTrailing) { Text(headerSummaryText) } }` 블록을 **완전히 삭제**한다. `.navigationTitle("오늘일정")`(v1.11 확정 문구)만 남긴다.
  - **순서 필수 이유**: 2단계를 1단계보다 먼저(또는 1단계 없이 단독) 적용하면 `headerSummaryText` 가 일시적으로 미사용 프로퍼티가 되고 P-73/AC-96 요건이 미충족 상태로 남는다. 1단계를 먼저 적용하면 이 문제가 없다.
  - **삭제하지 않음**: 계산 프로퍼티 `headerSummaryText` 자체(산식 무변경) — 1단계 적용 후 리스트 Section 헤더의 유일한 소비처가 된다.
  - **변경하지 않음(Developer 착수 전 실제 코드로 재확인)**: "다음 예정" `Section` 제거(v1.17, P-68 — 조사 결과 이미 제거된 상태), 빈 상태 문구 `Text("오늘 일정이 없습니다")`(E-19-5), `ScheduleRow.swift`(완료 토글 좌측 배치, P-69 — 텍스트 리터럴 없음, 이번 BLOCKED 보고 범위 밖이므로 실제 상태 재확인 권장).
- **최종 상태**: 화면 상단에는 `.navigationTitle("오늘일정")` 만 남고 우측 요약 없음(AC-91 폐기, 검증 대상 제외). 할일 카드 목록 바로 위 리스트 `Section` 헤더에는 "오늘일정 - {완료}/{전체}"가 표시된다(P-73, **AC-96 이번 릴리스 최초 검증**).
- **무변경**: 공유 페이로드 계약(`WatchSnapshot`/`WatchScheduleItem`), `WatchSyncService`/`WatchConnectivityGateway`/`resolveToggleLWW`, `ScheduleService`/`CategoryService` 등 모든 서비스·검증 로직, `bindings.ts`, DB 스키마(`database.md` v1.8 유지), RN 앱(`src/app/**`) 전체.
- **DB 스키마 변경**: 없음 — `database.md` v1.8 유지.
- **신규 의존성**: 없음.
- **NFR 문서**: `nfr.md` v1.18 — 정량 목표 변경 없음, §14.5 검증 문구를 "무변경 회귀 확인"에서 "P-73/AC-96 최초 구현 확인 + toolbar 삭제 확인" 동시 검증으로 정정.
- **재검증 필요 AC (Developer 구현 후 Tester)**: **AC-96**(리스트 Section 헤더 요약 "오늘일정 - {완료}/{전체}" — **이번 릴리스 최초 구현·최초 검증**, 회귀 아님) · **AC-47**(타이틀만 표시, 리스트 Section 헤더 요약은 별도 표시) · **AC-54**(빈 상태에 "오늘일정" 타이틀만, 요약 없음). **AC-91 은 검증 대상에서 제외**(폐기). 회귀 확인: **AC-92**(다음일정 부재+체크박스 좌측 — 무변경) · **AC-53**(워치 페이로드 범위 — 문구·필드 무관).
- **회귀 위험**: 낮음 — 순수 UI 요소 변경(라벨 전환 1개 + `.toolbar` 블록 삭제 1개)에 한정되며 상태·서비스·데이터 흐름을 건드리지 않는다. 유일한 주의점은 순서(1단계 먼저) 준수와, `headerSummaryText` 를 실수로 함께 삭제하지 않는 것 — `logic.md` §17.12 "구현 순서(v1.21, 필수)" 참고.

### v1.20 (F-19 개정(축소) — 워치 TodayView 상단 헤더 요약(`.toolbar`) 삭제) — **아래 지시는 v1.21 절로 대체됨(참고용으로 보존)**

- **변경(Developer) — 네이티브 워치 타깃(`ios/TodayWhatWatch/`)**:
  - `TodayView.swift`: 루트 뷰의 `.toolbar { ToolbarItem(placement: .topBarTrailing) { Text(headerSummaryText) } }` 블록을 **완전히 삭제**한다. `.navigationTitle("오늘일정")`(v1.11 확정 문구)만 남긴다.
  - **삭제하지 않음**: 계산 프로퍼티 `headerSummaryText`(`let done = ...`, `let total = ...`, `"오늘일정 - \(done)/\(total)"`) 자체는 그대로 유지한다 — v1.19에서 도입된 `Section(headerSummaryText)`(리스트 Section 헤더, P-73)가 유일한 소비처가 된다.
  - **변경하지 않음**: "다음 예정" `Section` 제거(v1.17, P-68 — 이미 제거된 상태 유지), `Section("오늘일정")` 라벨(v1.18), 빈 상태 문구 `Text("오늘 일정이 없습니다")`(E-19-5), `ScheduleRow.swift`(완료 토글 좌측 배치, P-69 — 텍스트 리터럴 없음).
- **무변경**: 공유 페이로드 계약(`WatchSnapshot`/`WatchScheduleItem`), `WatchSyncService`/`WatchConnectivityGateway`/`resolveToggleLWW`, `ScheduleService`/`CategoryService` 등 모든 서비스·검증 로직, `bindings.ts`, DB 스키마(`database.md` v1.8 유지), RN 앱(`src/app/**`) 전체.
- **DB 스키마 변경**: 없음 — `database.md` v1.8 유지.
- **신규 의존성**: 없음.
- **NFR 문서**: `nfr.md` v1.17 — 정량 목표 변경 없음, §9 V-58/§14.5 의 검증 체크리스트 문구만 실제 화면(상단 요약 부재, 리스트 Section 헤더 요약 존재)에 맞게 정정.
- **재검증 필요 AC (Developer 구현 후 Tester)**: **AC-47**(재정정 — 타이틀만 표시, 리스트 Section 헤더 요약은 별도 표시)**·AC-54**(재정정 — 빈 상태에 "오늘일정" 타이틀만, 요약 없음). **AC-91 은 검증 대상에서 제외**(폐기 — 상단 헤더 요약 기능 자체가 삭제됨). 회귀 확인: **AC-96**(리스트 Section 헤더 요약 "오늘일정 - {완료}/{전체}" — 무변경, 계속 표시되어야 함)**·AC-92**(다음일정 부재+체크박스 좌측 — 무변경)**·AC-53**(워치 페이로드 범위 — 문구·필드 무관).
- **회귀 위험**: 낮음. 순수 UI 요소 1개(`.toolbar` 블록) 삭제에 한정되며 상태·서비스·데이터 흐름을 건드리지 않는다. 유일한 주의점은 `headerSummaryText` 계산 프로퍼티를 실수로 함께 삭제하지 않는 것(리스트 Section 헤더가 계속 이를 참조하므로 삭제 시 컴파일 에러 또는 리스트 Section 헤더 요약 회귀 발생) — `logic.md` §17.12 에 명시.

### v1.18 (워치 TodayView 타이틀·헤더 요약 문구 "오늘"→"오늘일정"(v1.11 확정, 이번 라운드 반영) / RN 대시보드 탭 라벨 "오늘"→"오늘일정"(v1.12(A), OI-32 해소) / 워치 레이아웃 검증 결과 확인(v1.12(B), 변경 없음))

- **변경(Developer) — 네이티브 워치 타깃(`ios/TodayWhatWatch/`)**:
  - `TodayView.swift`: `.navigationTitle("오늘")` → `.navigationTitle("오늘일정")`. `headerSummaryText` 계산식의 `"오늘 - \(done)/\(total)"` → `"오늘일정 - \(done)/\(total)"`. 오늘 목록을 감싸는 `Section("오늘")` → `Section("오늘일정")`(설계 판단, 비필수 확장 — AC 미고정 순수 레이아웃 라벨).
  - **변경하지 않음**: 같은 파일의 빈 상태 문구 `Text("오늘 일정이 없습니다")`(E-19-5, plan.md AC 문구 불변 + 날짜 지시어), `ScheduleRow.swift`(텍스트 리터럴 없음).
- **변경(Developer) — 셸(`src/app`)**:
  - `src/app/navigation/RootNavigator.tsx`: `<Tab.Screen name={TAB_ROUTES.Dashboard} ... options={{ title: '오늘' }} />` 의 `title: '오늘'` → `title: '오늘일정'`. `TAB_FALLBACK_LABELS[TAB_ROUTES.Dashboard] = '오늘'` → `'오늘일정'`.
  - **변경하지 않음**: `TAB_ICONS[TAB_ROUTES.Dashboard]`(`todo.png` require 경로), `src/app/screens/DashboardScreen.tsx` 내부의 "오늘로" 복귀 버튼 텍스트·`formatDateLabel()` 의 `오늘 · {date}` 배지(F-20 날짜 네비게이션)·`dashboardViewModel.ts` 의 `DASHBOARD_EMPTY_TEXT`/`DASHBOARD_SEARCH_EMPTY_TEXT`(OI-33, 설계 결정 — §16.2 신설 소단락 근거).
- **무변경**: 공유 페이로드 계약(`WatchSnapshot`/`WatchScheduleItem`), `WatchSyncService`/`WatchConnectivityGateway`/`resolveToggleLWW`, `ScheduleService`/`CategoryService`/`assertValidScheduleInput` 등 모든 서비스·검증 로직, `bindings.ts`(신규 서비스 호출 없음), DB 스키마(`database.md` v1.8 유지), 탭 아이콘 파일 매핑.
- **워치 레이아웃 검증(v1.12(B))**: plan.md F-19 「레이아웃 검증 근거 (v1.12)」의 "완전히 일치, 변경 불요" 결론에 설계 관점 이견 없음(§17.11.3/§17.12 서술과 일치). 코드 변경 없음, `nfr.md` §14.5 에 회귀 참고자료 1줄만 추가.
- **DB 스키마 변경**: 없음 — `database.md` v1.8 유지.
- **신규 의존성**: 없음.
- **재검증 필요 AC (Developer 구현 후 Tester)**: **AC-47**(워치 타이틀·헤더 요약 리터럴)**·AC-54**(빈 상태 "오늘일정 - 0/0")**·AC-91**(헤더 요약 2/3→3/3 갱신)**·AC-95**(RN 탭 라벨·네비게이션 타이틀·폴백 텍스트 "오늘일정"). 회귀 확인: AC-53(워치 페이로드 범위 — 문구 무관), AC-92(다음일정 부재+체크박스 좌측 — 무변경), AC-26(탭 아이콘 매핑 — 무변경).
- **회귀 위험**: 낮음. 순수 리터럴 문자열 교체 4~5곳(워치 2~3곳 + RN 2곳)에 한정되며 상태·서비스·데이터 흐름을 건드리지 않는다. 유일한 주의점은 워치 `Section("오늘")` 라벨 변경이 `.carousel` 리스트 스타일에서 시각적으로 드러나는지 스크린샷으로 확인하는 것(§17.12).
...
- 워치 타이틀·헤더 요약 문구 정정 · RN 탭 라벨 문구 정정(F-19/F-10/F-16, v1.18): 신규 성능·용량·가용성·관측성 목표 없음 — 순수 리터럴 문자열 교체(빌드 타임 상수)이며 조회·집계·전송 경로에 영향이 없다. 상세 `nfr.md` §9 V-58·§14.5.
- 워치 리스트 Section 헤더 요약 추가(F-19, v1.19 설계·v1.21 구현) / 워치 상단 헤더 요약(`.toolbar`) 삭제(F-19, v1.20 설계·v1.21 구현): 신규 성능·용량·가용성·관측성 목표 없음 — 이미 계산되던 `summary` 값의 표시 위치만 조정하는 순수 UI 변경(최종적으로 렌더 대상 1개 감소). 조회·집계·전송 경로 무영향. `nfr.md` v1.18(§9 V-58·§14.5 검증 체크리스트 — "P-73/AC-96 최초 구현 확인"으로 정정, 정량 목표 변경 없음).
...
| OI-32 | RN 앱 대시보드 탭 라벨 "오늘"도 워치 TodayView 와 동일하게 "오늘일정"으로 바꿀지 (v1.11 신규) | **CLOSED (plan v1.12)** — 사용자 신규 요청("앱에서 메뉴 중 오늘을 오늘일정으로 변경해줘")으로 확정. `RootNavigator.tsx` `title`/`TAB_FALLBACK_LABELS` 2개 리터럴 교체(P-72, AC-95, logic §16.2, 영향 범위 "v1.18" 절). 미결정 아님 |
| OI-33 | `DashboardScreen.tsx` 내부의 다른 "오늘" 표현(날짜 네비게이션 "오늘로"/`오늘 · {date}`, 빈 상태 문구 등)까지 "오늘일정"으로 확장할지 (F-10, v1.12 신규, Architect/Developer 위임) | **이번 v1.18 라운드는 제외(설계 결정)** — 해당 표현은 탭·화면 정체성 명칭이 아니라 "당일"을 가리키는 날짜 지시어이며, plan.md 의 관련 서술·AC(E-10-1/AC-15 등)도 문구를 바꾸지 않았다(logic §16.2 신설 소단락 근거). 비차단, 후속 사용자 요청 시 별도 라운드에서 재검토 |
| N-17 | (신규, v1.18, note, 비차단) plan.md §11 OI 레지스트리의 OI-32 문구가 v1.12 갱신 시점에 CLOSED 로 갱신되지 않았고 OI-33 이 §11 목록 자체에 항목으로 추가되지 않음(변경이력 표·F-10 서술부·§5.20 서술에는 반영) | 정책 내용 자체는 3곳(변경이력 표 v1.12 행, F-10 서술부, §5.20 P-72 서술)에서 일관되게 명확히 기술되어 있어 설계를 차단하지 않고 PASS + note 로 처리(기획 검증 결과 "§11 OI 레지스트리 동기화 이슈" 참고). Planner 가 다음 라운드에서 plan.md §11 목록 자체를 동기화(OI-32 CLOSED 표기, OI-33 항목 추가)하도록 권장 |
| N-18 | (신규, v1.20, note, 비차단, **RESOLVED — plan.md v1.15**) plan.md AC-96(v1.13 신규, 1779행·1781행)의 Then/And 절이 v1.14에서 삭제된 상단 헤더 요약(P-68)이 여전히 표시되는 것처럼 서술("상단 네비게이션 타이틀 옆 헤더 요약... 동일한 수치로 함께 표시된다", "요약은 상단 헤더의 '오늘일정 - 0/0' 하나뿐이다")해, 같은 문서의 F-19 서술부·P-68·E-19-5·E-19-8·AC-47·AC-54·AC-91·§7.10(모두 v1.14 갱신 지점)과 상충함 | v1.20 당시에는 정책 내용 자체가 F-19 서술부·P-68·E-19-5·E-19-8·AC-47·AC-54·AC-91·§7.10 8곳에서 일관되게 "상단 헤더 요약 삭제"로 명확히 기술되어 있어 설계를 차단하지 않고 PASS + note 로 처리했다(기획 검증 결과 "v1.20 재검증" note 참고). **v1.15 에서 Planner 가 AC-96의 잔존 문장(세 번째 항목)을 상단 헤더 요약 삭제 사실에 맞춰 직접 정정 — 해소(RESOLVED).** logic.md §17.10 AC-96 매핑은 이미 삭제 사실을 반영하고 있어 이번 정정에 따른 추가 설계 변경은 없다(v1.22 재검증 "요구사항 충돌" 항목 참고) |
| N-19 | (신규, v1.21, note, 비차단, Architect 공정 개선) v1.19 logic.md 설계(`Section(headerSummaryText)`, P-73/AC-96)가 실제 Developer 구현·Tester 검증 사이클을 거치지 않은 채, 곧이은 v1.20 설계 라운드에서 "이미 구현됨"을 전제로 서술되어 Developer `BLOCKED(DESIGN_CONFLICT)` 로 이어짐(§14 일관성 표 v1.21 행 참고) | 이번 라운드는 실제 코드·git 워킹트리·`log/agent-execute.log` 대조로 원인을 확인하고 logic.md §17.1/§17.9/§17.10/§17.11.3/§17.12 서술 및 구현 지시를 정정해 해소했다. **재발 방지 권장(비차단, 정책 사항 아님)**: 이후 Architect 라운드는 같은 세션 안에서 이전 설계 버전의 서술을 다음 설계의 전제로 이어받기 전에, 해당 대상 파일의 실제 코드 상태(및 직전 라운드의 Developer/Tester RESULT 로그 존재 여부)를 먼저 확인할 것을 권장 |
